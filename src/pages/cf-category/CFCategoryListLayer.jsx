...
const initialCategories = [
  ...
  },
  {
    id: 2,
    name: "Electronics",
    description: "Gadgets, home appliances, and mobile accessories.",
    icon: "assets/images/categories/electronics.png",
    availability: "Inactive",
  },
  {
    id: 3,
    name: "Home Services",
    description: "Cleaning, plumbing, and electrical repairs.",
    icon: "assets/images/categories/services.png",
    availability: "Active",
  }
];
      ...
      <div className='card-header border-bottom bg-base py-16 px-24 d-flex align-items-center flex-wrap gap-3 justify-content-between'>
        <div className='d-flex align-items-center flex-wrap gap-3'>
          ...
          <form className='navbar-search'>
            <input type='text' className='bg-base h-40-px w-auto' name='search' placeholder='Search Categories...' />
            <Icon icon='ion:search-outline' className='icon' />
          </form>
        </div>
        <Link to='/add-cf-category' className='btn btn-primary text-sm btn-sm px-12 py-12 radius-8 d-flex align-items-center gap-2'>
                ...
                categories.map((category, index) => (
                  <tr key={category.id}>
                    ...
                    <td className='text-center'>
                      <span className={`px-12 py-4 radius-4 fw-medium text-sm ${category.availability === 'Active' ? 'bg-success-focus text-success-600 border border-success-main' : 'bg-danger-focus text-danger-600 border border-danger-main'}`}>
                        {category.availability}
                      </span>
                    </td>
                    <td className='text-center'>
                      <div className='d-flex align-items-center gap-10 justify-content-center'>
                        <Link to={`/view-cf-category/${category.id}`} className='bg-info-focus bg-hover-info-200 text-info-600 fw-medium w-40-px h-40-px d-flex justify-content-center align-items-center rounded-circle' title="View">
                          <Icon icon='majesticons:eye-line' className='icon text-xl' />
                        </Link>
                        <Link to={`/edit-cf-category/${category.id}`} className='bg-success-focus text-success-600 bg-hover-success-200 fw-medium w-40-px h-40-px d-flex justify-content-center align-items-center rounded-circle' title="Edit">
